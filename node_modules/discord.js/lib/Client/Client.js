...
var _StructuresPMChannel2 = _interopRequireDefault(_StructuresPMChannel); // This utility function creates an anonymous error handling wrapper function
...
var Client = (function(_EventEmitter) {
    ...
    /*
		...
		client.onde q vc ta com aquele codigo?
	*/
    function Client() {
    ...
    Client.prototype.sendTTSMessage = function sendTTSMessage(where, content) {
        ...
        return this.sendMessage(where, content, {
            tts: true
        }).then(dataCallback(callback), errorCallback(callback));
    }; // def reply
    ...
    Client.prototype.replyTTS = function replyTTS(where, content) {
        var callback = arguments.length <= 2 || arguments[2] === undefined ? function() /*err, msg*/ {} : arguments[2];
        return this.reply(where, content, {
            tts: true
        }).then(dataCallback(callback), errorCallback(callback));
    }; // def deleteMessage
    ...
    Client.prototype.setStatus = function setStatus(idleStatus, gameID) {
        var callback = arguments.length <= 2 || arguments[2] === undefined ? function() /*err, {}*/ {} : arguments[2];
        if (typeof gameID === "function") { // gameID is the callback
            callback = gameID;
        } else if (typeof idleStatus === "function") { // idleStatus is the callback
            callback = idleStatus;
        }
    ...
    Client.prototype.awaitResponse = function awaitResponse(msg) {
        ...
        var ret;
        if (toSend) {
            if (typeof toSend === "function") { // (msg, callback)
                callback = toSend;
            } else { // (msg, toSend, ...)
                if (options) {
                    if (typeof options === "function") { //(msg, toSend, callback)
                        callback = options;
                        ret = this.sendMessage(msg, toSend);
                    } else { //(msg, toSend, options, callback)
                        ret = this.sendMessage(msg, toSend, options);
                    }
                } else { // (msg, toSend) promise
                    ret = this.sendMessage(msg, toSend);
                }
            }
        }
